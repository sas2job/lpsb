...
WHERE 
ProductID IN (SELECT ProductID FROM Products WHERE UnitsInStock > 40) 
AND 
OrderID IN (SELECT OrderID FROM Orders WHERE Freight >= 50);

-- рефлексия (задание 7.3)
-- Научился использовать:
-- - вычисляемые поля;
-- - вложенный запрос для отбора данных;
